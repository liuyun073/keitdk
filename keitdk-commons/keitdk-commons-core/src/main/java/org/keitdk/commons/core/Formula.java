...
public class Formula {
	 ...
	 *            变量值
	 */
	public static Formula newInstance(int operator, String name, String value)
	 ...
	 *            变量值
	 */
	public static Formula newInstance(String name, int type, int operator, Object value)
			throws RuntimeException {
	...
	public String toString() {
		...
		if (value == null && operator != NU && operator != NN)
			...
			case IN:
				expression = name + " in ("
						+ ((type == FORMULA || type == SELECT) ? value : ":" + name) + ")";
				break;

			case NI:
				expression = name + " not in ("
						+ ((type == FORMULA || type == SELECT) ? value : ":" + name) + ")";
				break;
		...
		if (value != null) {
			...
			case BIGDECIMAL:
				if (!(value instanceof BigDecimal))
					throw new RuntimeException("变量" + name + "的类型定义为BigDecimal，实际赋值类型为"
							+ value.getClass().getName());
				break;

			case BIGINTEGER:
				if (!(value instanceof BigInteger))
					throw new RuntimeException("变量" + name + "的类型定义为BigInteger的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case BINARY:
				if (!(value instanceof Byte[]))
					throw new RuntimeException("变量" + name + "的类型定义为Binary的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case BOOLEAN:
				if (!(value instanceof Boolean))
					throw new RuntimeException("变量" + name + "的类型定义为Boolean的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case BYTE:
				if (!(value instanceof Byte))
					throw new RuntimeException("变量" + name + "的类型定义为Byte的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case DATE:
				if (!(value instanceof Date))
					throw new RuntimeException("变量" + name + "的类型定义为Date的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case DOUBLE:
				if (!(value instanceof Double))
					throw new RuntimeException("变量" + name + "的类型定义为Double的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case FLOAT:
				if (!(value instanceof Float))
					throw new RuntimeException("变量" + name + "的类型定义为Float的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case INTEGER:
				if (!(value instanceof Integer))
					throw new RuntimeException("变量" + name + "的类型定义为Integer的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case LONG:
				if (!(value instanceof Long))
					throw new RuntimeException("变量" + name + "的类型定义为Long的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case SHORT:
				if (!(value instanceof Short))
					throw new RuntimeException("变量" + name + "的类型定义为Short的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case STRING:
				if (!(value instanceof String))
					throw new RuntimeException("变量" + name + "的类型定义为String的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case TEXT:
				if (!(value instanceof String))
					throw new RuntimeException("变量" + name + "的类型定义为Text的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case TIME:
				if (!(value instanceof Date))
					throw new RuntimeException("变量" + name + "的类型定义为Time的实际赋值类型为"
							+ value.getClass().getName());
				break;

			case TIMESTAMP:
				if (!(value instanceof Date))
					throw new RuntimeException("变量" + name + "的类型定义为TimeStamp的实际赋值类型为"
							+ value.getClass().getName());
